...
class GoodsItem extends React.Component {
  ...
  clearInterval = () => {
    if (this.type1Timer) {
      clearInterval(this.type1Timer);
      this.type1Timer = false;
    }
  };

  render() {
    const { goods, history, isNewcommerChannel, isReduction } = this.props;
    //isReduction 满减活动列表   isNewcommerChannel新人频道列表
    const { now_time } = this.state;
    let is_secondkill =
      goods.activity === 2 &&
      goods.activity_info &&
      ...
      goods.activity_info.end_time > now_time
        ...
        : false;
    return (
      <div className='--goods-item-type1'>
        {isNewcommerChannel && (
        ...
        <div>
          <div
            className='goods-name line-clamp-two'
            onClick={() => {
              this.toDetailPage(goods.id);
            }}
          >
